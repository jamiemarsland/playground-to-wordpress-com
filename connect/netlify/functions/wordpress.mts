...
function page(title, body, status = 200, extra = [], bridge = false) {
  ...
  return new Response(`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escape(title)} | Playground Connect</title><style>body{font:17px/1.65 system-ui,sans-serif;background:#f0f4f6;color:#142b33;margin:0;padding:8vh 20px}main{max-width:620px;margin:auto;background:white;padding:38px;border-radius:16px;border-top:5px solid #155b4b}h1{font-size:30px;line-height:1.2}h2{font-size:21px;margin:0}section{border:1px solid #d9e4df;border-radius:8px;padding:20px;margin:20px 0}li{padding-left:4px}strong{overflow-wrap:anywhere}a{color:#155b4b}button,.button{display:inline-block;background:#155b4b;color:white;padding:12px 20px;border:0;border-radius:6px;font:inherit;text-decoration:none;cursor:pointer}code{display:block;background:#eef3f1;padding:16px;overflow-wrap:anywhere;font-size:14px}small{color:#52635e}a:focus-visible,button:focus-visible{outline:3px solid #a66b00;outline-offset:4px}</style><main><small>PUT YOUR SITE ONLINE</small><h1>${escape(title)}</h1>${body}</main>${bridge ? '<script src="/transfer.js" defer></script>' : ''}</html>`,{status,headers:h});
}
