...
function page(title, body, status = 200, extra = [], bridge = false) {
  ...
  return new Response(`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escape(title)} | Playground Connect</title><style>:root{--wp-blue:#3858e9;--wp-blue-dark:#1d35b4;--ink:#101517;--muted:#50575e;--line:#dcdcde}body{font:16px/1.65 -apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Inter,sans-serif;background:#f6f7f7;color:var(--ink);margin:0;padding:7vh 20px}main{max-width:620px;margin:auto;background:#fff;padding:40px;border-radius:12px;border:1px solid var(--line)}small.kick{display:block;font-size:11px;letter-spacing:.16em;font-weight:700;text-transform:uppercase;color:var(--muted)}h1{font-size:30px;line-height:1.15;font-weight:700;letter-spacing:-.02em;margin:12px 0 18px}h2{font-size:19px;font-weight:700;margin:0 0 8px}p,li{color:var(--muted)}p strong{color:var(--ink)}section{border:1px solid var(--line);border-radius:8px;padding:22px;margin:22px 0}li{padding-left:4px}strong{overflow-wrap:anywhere}a{color:var(--wp-blue)}button,.button{display:inline-block;background:var(--wp-blue);color:#fff;padding:12px 22px;border:0;border-radius:4px;font:inherit;font-weight:600;text-decoration:none;cursor:pointer}button:hover,.button:hover{background:var(--wp-blue-dark)}code{display:block;background:#f6f7f7;border:1px solid var(--line);border-radius:4px;padding:16px;overflow-wrap:anywhere;font-size:13px}small{color:#787c82}progress{width:100%;height:8px;border-radius:999px;overflow:hidden}a:focus-visible,button:focus-visible{outline:2px solid var(--wp-blue);outline-offset:3px}</style><main><small class="kick">PUT YOUR SITE ONLINE</small><h1>${escape(title)}</h1>${body}</main>${bridge ? '<script src="/transfer.js" defer></script>' : ''}</html>`,{status,headers:h});
}
